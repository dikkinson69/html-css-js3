...
class Car {
  ...
  stopCar() {
    if (!this.started) {
      ...
      this.started = false;
      console.log(`${this.model} has been stopped`);
    }
  }
...
document.getElementById("driveCarBtn").addEventListener("click", () => {
  if (selectedCar) {
    carInfo.textContent = selectedCar.drive(); 
  } else {
    alert('select a car first');
  }
});
...
document.getElementById("stopCarBtn").addEventListener("click", () => {
  if (selectedCar) {
    carInfo.textContent = selectedCar.stopCar();
  } else {
    alert('Select a car first');
  }
});